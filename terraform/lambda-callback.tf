...
resource "aws_lambda_function" "callback_lambda" {
  ...
  environment {
    variables = {
      ...
      SPOTIFY_CLIENT_SECRET_PARAM           = aws_ssm_parameter.spotify_client_secret.name
      SPOTIFY_PRIMARY_REFRESH_TOKEN_PARAM   = aws_ssm_parameter.spotify_primary_refresh_token.name
      SPOTIFY_SECONDARY_REFRESH_TOKEN_PARAM = aws_ssm_parameter.spotify_secondary_refresh_token.name
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          aws_ssm_parameter.spotify_primary_refresh_token.arn,
          aws_ssm_parameter.spotify_secondary_refresh_token.arn,
        ]
      },
      {
        Effect   = "Allow"
